...
package BallerinaOpenPrEmailer;
...
public function main (string[] args) {
    ...
    string sender = config:getGlobalValue("BALLERINA_GRP_EMAIL_SENDER");
    sender = sender.subString(1,(lengthof sender)-1);
    string receiver = config:getGlobalValue("BALLERINA_GRP_EMAIL_RECEIVER");
    receiver = receiver.subString(1,(lengthof receiver)-1);
    string cc = config:getGlobalValue("BALLERINA_GRP_EMAIL_CC");
    cc = cc.subString(1,(lengthof cc)-1);
    string title = config:getGlobalValue("BALLERINA_GRP_EMAIL_TITLE");
    title = title.subString(1,(lengthof title)-1);
    string orgs = config:getGlobalValue("BALLERINA_GRP_EMAIL_ORGS");
    ...
    logFile:logDate("BallerinaOpenPrEmailer");
    ...
    string emailContent = emailer:generateBallerinaGroupEmail(tableContent);
